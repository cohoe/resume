...
%\emph{Web:} \href{http://www.grantcohoe.com/}{http://grantcohoe.com}}
...
\item[] B.S.,
        \href{http://www.nssa.rit.edu/}
             ...
             \hfill GPA: 3.23/4.0
        \begin{innerlist}
        \item Minors in \href{http://www.rit.edu/programs/minor_conc/database-design-and-development}
...
\section{Skills}
\textbf{Management Tools}: Ansible, Puppet, VMware vSphere, Cisco UCS Manager, EMC Unisphere

\textbf{Cloud Providers}: Amazon Web Services, Microsoft Azure, OpenStack, VMware vCloud \& vCloud Air

\textbf{Network}: Cisco Catalyst \& Nexus Switching, JunOS, F5, IronPort WSA, VPN (IPsec, AnyConnect)

\textbf{Services \& Applications}: OpenLDAP, ISC-BIND/DHCPD, Jenkins, Github Enterprise, Docker

\textbf{System Platforms}: Linux (Focus: RHEL/SLES), Windows Server (2003-2016)

\textbf{Development}: Python, Bash, Git, Perforce

%A complete list of skills can be found at  \href{http://www.grantcohoe.com/skills} {http://grantcohoe.com/skills}

\section{Projects}

\textbf{Bluetooth Headphone Connection Manager}
\begin{outerlist}
    \item[]
            \begin{innerlist}
                \item Tool in Python on Fedora Linux to manage PulseAudio sinks and deal with Bluetooth connectivity quirks.
                \item Listens for DBus events and routes audio stream based on user configuration and connection state.
            \end{innerlist}
\end{outerlist}
\halfblankline

\textbf{College Hockey Scheduling API} (In Progress)
\begin{outerlist}
    \item[]
            \begin{innerlist}
                \item Designed and implemented a RESTful API using DynamoDB, Lambda, and API Gateway (AWS).
                \item Uses the \href{http://www.serverless.com/}{Serverless} framework to provide high scalability with no persistent infrastucture.
            \end{innerlist}
\end{outerlist}
\halfblankline

\textbf{Pipeline-based Package Mirror}
\begin{outerlist}
    \item[]
            \begin{innerlist}
                \item Developed an RPM HTTP-only mirror system using \href{https://github.com/constantcontact/jenkins_pipeline_builder}{Jenkins Pipeline Builder} and Python.
                \item Jobs defined as JSON so service is fully deployable from VCS (only Jenkins installation required).
            \end{innerlist}
\end{outerlist}
\halfblankline
...
\href{http://www.constantcontact.com/}{\textbf{Constant Contact}},
Waltham MA
\begin{outerlist}
\item[] \textit{Senior System Engineer}%
        \hfill \textbf{Jan 2017 to Present}
\begin{innerlist}
...
\begin{outerlist}
 \item[]
            \begin{innerlist}
			\item Amateur Ice Hockey Player
...
%		\item Frequent TEDx \& Barcamp Attendee
		\item Outdoor Excursion Enthusiast
	                \item Mens \& Womens College/Pro Hockey Fan
            \end{innerlist}
\end{outerlist}
\end{document}
